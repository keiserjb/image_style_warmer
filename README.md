...
that all matching permanent image files are processed.
...
The module provides a file action named **Warm all image styles**. On the
Manage files page, select one or more image files, choose this action, and
click **Execute**. It synchronously generates every defined image style for
each selected file, skipping derivatives that already exist when that option
is enabled.
